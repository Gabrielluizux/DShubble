...
// Every "bg-{family}-{step}" string below must stay literal so Tailwind's scanner can
// find and generate it -- a dynamic template like `bg-${family}-${step}` would not be detected.
...
function PaletteRow({ name, classes }: { name: string; classes: string[] }) {
  return (
      ...
      <div style={{ display: "flex" }}>
        {classes.map((cls, i) => (
          <div key={cls} style={{ textAlign: "center" }}>
            <div className={cls} style={{ width: 44, height: 44, border: "1px solid var(--color-border-100)" }} />
            <div style={{ fontSize: 9, marginTop: 4 }}>{steps[i]}</div>
          </div>
        ))}
      </div>
    </div>
...
const meta = {
  title: "Foundations/Colors/Tailwind Palette",
  tags: ["autodocs"],
  ...
  render: () => (
    <div>
      <p style={{ fontSize: 13, color: "var(--color-muted-text)", marginBottom: 24, maxWidth: 640 }}>
        Tailwind CSS&apos;s full default palette (
        <a href="https://tailwindcss.com/docs/colors" target="_blank" rel="noreferrer">
          reference
        </a>
        ), distinct from hubble-ds&apos;s own Brand Palette. Use this only when a design
        genuinely needs a color combination outside what the DS tokens cover — e.g. extra
        badge/indicator states — not as a substitute for semantic tokens.
      </p>
      {families.map((f) => (
